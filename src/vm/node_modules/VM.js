...
function isCIDR(str) {
    ...
    var parts = str.split('/');
    if (parts.length !== 2 || (!net.isIPv4(parts[0]) && !net.isIPv6(parts[0]))) {
        return false;
    }

    var size = Number(parts[1]);
    if (size === null || size === undefined || size === false || 
            (net.isIPv4(parts[0]) && (size < 0 || size > 32)) ||
	    (net.isIPv6(parts[0]) && (size < 0 || size > 128))) {
        return false;
    }
...
// This is for allowed_ips which accepts IP addresses or CIDR addresses in
// the form IP/MASK where MASK is 1-32 (or 1-128 for v6).
function validateIPlist(list) {
    ...
    list.forEach(function (ip) {
        ...
        if (!net.isIPv4(ip) && !net.isIPv6(ip)) {
            matches = ip.match(/^([0-9a-fA-F\.\:]+)\/([0-9]+)$/);
            if (matches && ((net.isIPv4(matches[1]) 
                && (Number(matches[2]) >= 1) && (Number(matches[2]) <= 32)) ||
		(net.isIPv6(matches[1]) && (Number(matches[2])) >= 1 && (Number(matches[2])) <= 128))) {

                // In this case it wasn't an IP, but it was a valid CIDR
                return;
            } else {
                invalid.push(ip);
            }
        ...
        function (cb) {
            ...
            if (options.hasOwnProperty('fields')
                && (options.fields.indexOf('server_uuid') === -1
                    ...
                    && options.fields.indexOf('platform_buildstamp') === -1
    ...
    getVmobj(info.zonename, data, options, function (err, vmobj) {
        ...
        if (data.hasOwnProperty('sysinfo')) {
            ...
            if (wantField('platform_buildstamp')
                && data.sysinfo.hasOwnProperty('Live Image')) {

                vmobj.platform_buildstamp = data.sysinfo['Live Image'];
            }
